...
| | Desktop (`classic`) | Desktop (`type: 'redirect'`) | Mobile (any `type`) | In-app WebView (FB/IG/TikTok) |
| --- | --- | --- | --- | --- |
| **What the user sees after the click** | Their content, the offer in a tiny window behind | The offer in the current tab | Their content in a new tab | Nothing, the clickunder stays silent |
| **Where the offer is** | Expands on the next click | Current tab | The previous (original) tab | Not opened |
| **Where the original link is** | Current tab (the click is replayed) | New tab | New foreground tab | In place |
| **If the browser blocks the popup** | Nothing happens | Nothing happens | Nothing happens | — |

Why not a classic popunder on mobile: phones have no "window behind" — every tab is fullscreen, and `window.open` from a gesture opens a new tab and immediately activates it. Hence the tab swap: open the content first (it lands in front of the user's eyes), then send the current tab to the offer. Works in Chrome Android, iOS Safari and Firefox.

iPadOS 13+ pretends to be a desktop Mac in its UA, so we detect it via `navigator.maxTouchPoints` — otherwise it would get the desktop branch with a blocked popup.
